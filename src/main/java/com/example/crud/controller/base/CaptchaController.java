...
// import com.example.crud.util.CaptchaUtil;
...
public class CaptchaController extends BaseController {
	...
	public void genCaptcha(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 在内存中创建图象 create image in memory
		BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		// 获取图形上下文 Get the graphics context
		Graphics g = image.getGraphics();
		// 生成随机类 generate random class
		Random random = new Random();
		// 设定背景色 set background color
		g.setColor(new Color(255, 255, 255));
		g.fillRect(0, 0, width, height);
		// 设定字体 set font
		g.setFont(font);
		// 取随机产生的验证码 Take a randomly generated verification code
		String sRand = "";
		//文字X坐标 Text X coordinate
		int codeX = width/(length+1);
		for (int i = 0; i < length; i++) {
			String rand = String.valueOf(getRandomText());
			sRand += rand;
			//将验证码输出到图象中 Output the verification code into the image
			g.setColor(getRandColor(100, 150));// 调用函数出来的颜色相同  The color of the calling function is the same
			g.drawString(rand, codeX * i + 10, height-20);
		}
		// 生成干扰线 generate interference lines
		if (crossLine) {
